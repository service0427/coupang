-- 9단계: proxy_server 컬럼 순서 변경 (is_vmware 다음으로)
-- ip_type을 proxy_server로 변경한 후 컬럼 순서 재배치

BEGIN;

-- 새로운 테이블 생성 (원하는 컬럼 순서로)
CREATE TABLE test_keywords_new (
    ...
    os_type VARCHAR(20) NOT NULL DEFAULT 'win11',
    is_vmware BOOLEAN NOT NULL DEFAULT false,
    proxy_server VARCHAR(255),  -- is_vmware 다음으로 이동
...
SELECT 
    ...
    os_type, is_vmware, proxy_server, ip_change_enabled, allow_duplicate_ip,
...
-- 인덱스 재생성
CREATE INDEX idx_test_keywords_active ON test_keywords(is_active);
CREATE INDEX idx_test_keywords_browser ON test_keywords(browser);
CREATE INDEX idx_test_keywords_date ON test_keywords(date);
CREATE INDEX idx_test_keywords_os_vmware ON test_keywords(os_type, is_vmware);
...
COMMENT ON COLUMN test_keywords.os_type IS '운영체제 타입 (win11/u24/u22)';
COMMENT ON COLUMN test_keywords.is_vmware IS 'VMware 가상머신 실행 여부';
...
COMMENT ON COLUMN test_keywords.is_active IS '활성화 상태';
