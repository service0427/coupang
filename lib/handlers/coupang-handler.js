        ...
        if (productLink) {
          ...
          if (response && response.ok()) {
            ...
            if (cartClickEnabled && browserType !== 'webkit') {
              ...
              console.log(`   프록시 사용: ${proxyConfig ? proxyConfig.server : '직접 연결'}`);
              ...
              try {
                ...
                try {
                  ...
                  console.log(`   카운트 확인 오류: ${e.message}`);
                }
                
                // 장바구니 버튼 찾기
                console.log('   장바구니 버튼 검색 중...');
                ...
                if (cartButton && await cartButton.isVisible()) {
                  ...
                  console.log(`   버튼 상태: ${JSON.stringify(buttonState)}`);
                  ...
                  if (browserType === 'webkit') {
                    ...
                    const networkPromise = page.waitForResponse(response => 
                      ...
                      console.log(`   ⚠️ 장바구니 API 응답 대기 시간 초과: ${e.message}`);
                      return null;
                    });
                    ...
                    if (response) {
                      ...
                      if (!response.ok()) {
                        ...
                        console.log(`   ❌ API 에러 응답: ${responseText.substring(0, 200)}`);
                      }
                    }
                  }
                  ...
                  try {
                    ...
                    console.log(`   카운트 확인 실패: ${e.message}`);
                    console.log('   다른 방법으로 확인...');
                  ...
                  console.log(`   페이지의 버튼 분석: ${JSON.stringify(buttonAnalysis)}`);
