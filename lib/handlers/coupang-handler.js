  ...
  try {
    ...
    if (searchMode) {
      ...
      try {
        ...
        // WebKit의 경우 네비게이션 대기 및 차단 감지
        if(browserType.toUpperCase() == 'WEBKIT') {
          try {
            await Promise.all([
            ...
            console.log(`⏳ [${browserType.toUpperCase()}] 검색 결과 페이지 로딩 안정화를 위해 5초 대기...`);
            await page.waitForTimeout(5000);
            
          } catch (navError) {
            console.log(`⚠️ [${browserType.toUpperCase()}] 네비게이션 실패:`, navError.message);
            ...
            const pageState = await page.evaluate(() => {
              return {
                ...
                searchInputExists: !!document.querySelector('input[name="q"]'),
                hasSearchResults: !!document.querySelector('#product-list')
              };
            });
            
            console.log(`🔍 [${browserType.toUpperCase()}] 페이지 상태 확인:`);
            console.log(`   URL: ${pageState.url}`);
            console.log(`   제목: ${pageState.title}`);
            console.log(`   검색창 존재: ${pageState.searchInputExists}`);
            console.log(`   검색 결과: ${pageState.hasSearchResults}`);
            
          }

        } else {
          // Chrome/Firefox는 기존 방식
          await page.keyboard.press('Enter');
          console.log(`⏳ [${browserType.toUpperCase()}] 검색 결과 페이지 로딩 안정화를 위해 3초 대기...`);
          await page.waitForTimeout(3000);
        }
      ...
      try {
        ...
        console.log(`⏳ [${browserType.toUpperCase()}] 페이지 로딩 안정화를 위해 [WEBKIT: 10초, 나머지: 3초] 대기...`);
        // 검색 결과 페이지 로드 대기
        if(browserType.toUpperCase() == 'WEBKIT') {
