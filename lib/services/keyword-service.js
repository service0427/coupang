...
class KeywordService {
    ...
    try {
      let query = `
        SELECT 
          ...
          os_type, is_vmware, proxy_server, 
      ...
      if (options.proxyServer !== undefined) {
        if (options.proxyServer === null) {
          query += ` AND proxy_server IS NULL`;
        } else {
          query += ` AND proxy_server = $${paramIndex}`;
          params.push(options.proxyServer);
          paramIndex++;
        }
      }
   ...
   */
  async getKeywordStats(agent = null) {
    try {
      let query = `
        SELECT 
          agent,
      ...
      if (agent) {
        conditions.push(`(agent = $${params.length + 1} OR agent IS NULL)`);
        params.push(agent);
      }
  ...
  /**
   * 전체 활성 키워드 수 조회 (agent별)
   * @param {string} agent - 에이전트 이름
   * @returns {number} 활성 키워드 수
   */
  async getActiveKeywordCount(agent = null) {
    try {
      let query = `
      ...
      if (agent !== null) {
        params.push(agent);
        query += ` AND (agent = $${params.length} OR agent IS NULL)`;
      }
    ...
    try {
      const query = `
        INSERT INTO execution_logs (
          keyword_id, agent, success, product_found, product_rank, url_rank,
          pages_searched, cart_clicked, error_message,
          duration_ms, browser_used, proxy_used, actual_ip, final_url, search_query
      ...
      const params = [
        ...
        logData.actualIp || null,
        logData.finalUrl || null,
        logData.searchQuery || null
      ];
  ...
  async getExecutionLogs(keywordId, limit = 10) {
    try {
      const query = `
        SELECT 
          id, executed_at, agent, success, product_found, product_rank, url_rank,
          pages_searched, cart_clicked, error_message,
          duration_ms, browser_used, proxy_used, actual_ip, final_url, search_query
        FROM execution_logs
        WHERE keyword_id = $1
